        ...
        while True:
            ...
            if len(lmlist) != 0:
                ...
                if 15 < angle1 < 24:
                    if angle2 > 100:
                        direction = "down"
                    if angle2 < 41 and direction == 'down' and lmlist[15][1] > lmlist[23][1] and lmlist[15][1] - \
                            lmlist[11][1] < 20:
                        direction = "up"
                        count += 1
...
if __name__ == "__main__":
    ...
    app = QtWidgets.QApplication(sys.argv)
    window = QtWidgets.QMainWindow()
    ui = Ui_MainWindow("static/TestVideosAndImages/squats.gif")  # Provide a GIF path here
    ui.setupUi(window)
    window.show()
    form_checker.gif_window = window  # Store the reference to the GIF window

    # Start a separate thread for tracking
    tracking_thread = threading.Thread(target=form_checker.track_squats)
    tracking_thread.start()
